...
var Datetime = createClass({
	...
	getStateFromProps: function (props) {
		...
		if (selectedDate) {
			...
			if (this.props.buddishEra) {
				inputValue = selectedDate.format(formats.datetime.replace(/y/ig, '')) + (parseInt(selectedDate.format('YYYY'), 10) + 543);
			}
		}
	...
	updateSelectedDate: function (e, close) {
		...
		if (!this.props.value) {
			...
			var displayInputValue = date.format(this.state.inputFormat);
			if (this.props.buddishEra) {
				displayInputValue = date.format(this.state.inputFormat.replace(/y/ig, '')) + (parseInt(date.format('YYYY'), 10) + 543);
			}
	...
	render: function () {
		...
		if (this.props.input) {
			var finalInputProps = assign({ type: 'text', className: 'form-control', value: this.state.inputValue },
		...
		return React.createElement(ClickableWrapper, { className: className, onClickOut: this.handleClickOutside }, children.concat(
			React.createElement('div',
				...
				React.createElement(CalendarContainer, { view: this.state.currentView, viewProps: this.getComponentProps(), buddishEra: this.props.buddishEra })
			)
		));
	}
});
